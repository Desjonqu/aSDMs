...
reg_mapplot <- ggplot() + geom_raster(data = datosv, aes(x=x, y=y, fill = suitability)) + coord_quickmap(xlim = c(-9, 4), ylim = c(36, 44))+ scale_fill_viridis_c(option="D") + borders("world") + theme(legend.title = element_blank(),axis.title = element_blank()) + facet_wrap(~ month, nrow=3, ncol=4)+  geom_polygon(data=inter, aes(long, lat, group = group), colour = "black", fill = NA, size=0.25)
...
bou_mapplot <- ggplot() + geom_raster(data = datosv, aes(x=x, y=y, fill = suitability)) + coord_quickmap(xlim = c(-9, 4), ylim = c(36, 44))+ scale_fill_viridis_c(option="D") + borders("world") + theme(legend.title = element_blank(),axis.title = element_blank()) + facet_wrap(~ month, nrow=3, ncol=4)+  geom_polygon(data=inter, aes(long, lat, group = group), colour = "black", fill = NA, size=0.25)
