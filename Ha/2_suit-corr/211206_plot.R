...
mean.zphot <- mean(xdata$z.ph.ch[which(xdata$calling.freq!=0)])#mean of the ph.ch period when they and calling
mean.ztemp <- mean(xdata$z.temp[which(xdata$calling.freq!=0)])#mean of the ph.ch period when they and calling
mean.zprec <- mean(xdata$z.prec[which(xdata$calling.freq!=0)])#mean of the ph.ch period when they and calling
...
par(mfrow=c(2,3), mar=c(4.1, 4.1, 3, 0.8), oma=c(0,0.5,0,0))
...
plot(boot.res$ci.predicted$z.temp[mean.prec], boot.res$ci.predicted$fitted[mean.prec], type='l', ylim=ylim, xaxt='n', xlab="monthly minimum temperature (°C)", ylab="calling probability", cex.lab=1.6, cex.axis=1.2)
at=seq(-1.5, 1.5, 0.5)
...
plot(boot.res$ci.predicted$z.ph.ch[mean.prectemp], boot.res$ci.predicted$fitted[mean.prectemp], type='l', ylim=ylim, xaxt='n', xlab="monthly change in photoperiod (min.)", ylab='', cex.lab=1.6, cex.axis=1.2, main='Regression model', cex.main=2)
at=seq(-1.5, 1.5, 0.5)
...
plot(boot.res$ci.predicted$z.prec[mean.temp], boot.res$ci.predicted$fitted[mean.temp], type='l', ylim=ylim, xaxt='n', xlab="monthly cumulated precipitation (mm)", ylab="", cex.lab=1.6, cex.axis=1.2)
at=seq(-1.5, 2, 0.5)
...
plot(((xdata$pcmax+xdata$pcmin)/2)*60, (xdata$tmax+xdata$tmin)/2, ylim=range(c(xdata$tmax,xdata$tmin, res$call_tmax, res$call_tmin)), xlab='monthly change in photoperiod (min.)', ylab='monthly minimum temperature (°C)', main='Boundary model', col='black', bg=col, lwd=0.3, pch=pch, cex.lab=1.6, cex.axis=1.2, cex.main=2)
rect(xleft=res$call_phmin*60, ybottom=res$call_tmin, xright=res$call_phmax*60, ytop=res$call_tmax)

mtext(text = c("(a)", "(b)", "(c)", "(d)"), side = 1, at = c(-11, -3.2, 4.5, -3.2), line = c(-42, -42, -42, -18.2), font = 3)
